...
from main_model_utils import FocalLoss
...
class ModifiedViTLayer(ViTLayer):
    def __init__(self, config, sim_threshold=0.9, mlp_threshold=0.5, avg_threshold=0.1, mlp_needed=True):
    ...
    def forward(self, hidden_states: torch.Tensor,
        ...
        output_attentions: bool = False, compute_cosine = False, output_mask=False, previous_mask=None):

        if previous_mask:
            neighbours = hidden_states[:, indices, :]
            neighbours_mean = (neighbours * (not previous_mask.unsqueeze(-1).unsqueeze(-1))).mean(dim=2)

            hidden_states = hidden_states * (1 - self.avg_threshold) + neighbours_mean * self.avg_threshold
        ...
        # mask the edges
        # edge_indices = torch.cat((torch.arange(2, 14), torch.arange(1, 197, 14), torch.arange(14, 197, 14), torch.arange(184, 196)))
        # boolean_mask[:, edge_indices] = True
        ...
        labels = (similarity_val < self.sim_threshold)
        labels = torch.cat((cls_col, labels), dim=1)
        boolean_mask = labels
        # ---------------------------------------------------------------
        ...
        for i in range(batch_size):
            ...
            output[i][boolean_mask[i]] = super().forward(hidden_states[i].unsqueeze(0))[0].squeeze(0)[boolean_mask[i]]
            # output[i] = super().forward(hidden_states[i].unsqueeze(0))[0].squeeze(0)
        ...
        if self.training or compute_cosine:
            ...
            labels = boolean_mask[:, 1:].float()
            focal_alpha = labels.mean()
            mlp_criterion = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([focal_alpha / (1 - focal_alpha + 1e-16)]).to(boolean_mask.device))
            # mlp_criterion = nn.BCEWithLogitsLoss()
            # mlp_criterion = FocalLoss(alpha=.7)
            self.loss = mlp_criterion(mlp_output.squeeze(-1), labels)
            self.mlp_accuracy_arr = ((self.sim_threshold - similarity_val) * (boolean_mask[:, 1:].float() - .5) > 0)

            true_labels = (similarity_val < self.sim_threshold).int().cpu().flatten()
            pred_labels = boolean_mask[:, 1:].int().cpu().flatten()
...
class ModifiedViTEncoder(ViTEncoder):
    def __init__(self, config: ViTConfig, sim_threshold=0.9, mlp_threshold=0.5, avg_threshold=0.1):
        super().__init__(config)
        ...
        self.layer = nn.ModuleList([ModifiedViTLayer(config, sim_threshold, mlp_threshold, avg_threshold, mlp_needed_arr[_]) for _ in range(config.num_hidden_layers)])
    ...
    def forward(
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                layer_outputs = layer_module(hidden_states, layer_head_mask, output_attentions, compute_cosine=compute_cosine, output_mask=output_mask, previous_mask=boolean_mask)
            ...
            if output_mask:
                boolean_mask = layer_outputs[1]
                all_boolean_mask = all_boolean_mask + (boolean_mask,)
...
class ModifiedViTModel(ViTModel):
    def __init__(self, config: ViTConfig, sim_threshold=0.9, mlp_threshold=0.5, avg_threshold=0.1):
        super().__init__(config)
        self.encoder = ModifiedViTEncoder(config, sim_threshold, mlp_threshold, avg_threshold)
