...
class ModifiedViTLayer(ViTLayer):
    def __init__(self, config, compress, sim_threshold=0.9, mlp_threshold=0.5, mlp_needed=True):
    ...
    def forward(self, hidden_states: torch.Tensor,
        ...
        compressed_tokens = self.compress(tokens)
        flattened_tokens = compressed_tokens.view(compressed_tokens.shape[0], -1)

        # mlp_input = mlp_input.view(mlp_input.shape[0], -1)

        mlp_output = self.mlp_layer(flattened_tokens)
...
class ModifiedViTEncoder(ViTEncoder):
    def __init__(self, config: ViTConfig, sim_threshold=0.9, mlp_threshold=0.5):
        ...
        self.layer = nn.ModuleList([ModifiedViTLayer(config, self.compress, sim_threshold, mlp_threshold, mlp_needed_arr[_]) for _ in range(config.num_hidden_layers)])
