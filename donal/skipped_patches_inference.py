...
# Create a directory for skipped patch heatmaps
skipped_heatmap_dir = "skipped_heatmaps"
os.makedirs(skipped_heatmap_dir, exist_ok=True)

# Initialize dictionary to store per-layer skipped patch counts
layer_skipped_patch_counts = {layer_idx: np.zeros(num_patches, dtype=int) for layer_idx in range(len(model.encoder.layer))}
    ...
    for batch_idx, batch in enumerate(test_loader):
        ...
        for layer_idx, layer in enumerate(model.encoder.layer):
            if isinstance(layer, ModifiedViTLayer):
                # Get the pred_labels tensor which should contain information about skipped patches
                pred_labels = layer.pred_labels.cpu().numpy()  # Shape: (batch_size, 196)
                
                #  skipped patches are marked with 0 in pred_labels
                skipped_patches = (pred_labels == 0)
                
                batch_size = inputs.shape[0]
                skipped_patches = skipped_patches.reshape(batch_size, num_patches)
                
                # Sum over the batch dimension to count per-patch skips
                layer_skipped_patch_counts[layer_idx] += skipped_patches.sum(axis=0)
        
        # Optional: Print progress
        if (batch_idx + 1) % 10 == 0:
            print(f"Processed {(batch_idx + 1) * test_batch_size} samples...")

# Normalize and create heatmaps for skipped patches
for layer_idx, skipped_count in layer_skipped_patch_counts.items():
    # Normalize by the total number of samples
    normalized_skipped = skipped_count / test_size
    
    # Reshape into 14x14 grid for visualization
    skip_map = normalized_skipped.reshape(14, 14)
    ...
    # Save the heatmap
    heatmap_path = os.path.join(skipped_heatmap_dir, f"layer_{layer_idx}_skipped_heatmap.png")
    plt.savefig(heatmap_path)
    plt.close()

# Create combined visualization (optional) - skipped vs. total patches
print(f"Saved all skipped patch heatmaps in '{skipped_heatmap_dir}' directory.")

# Optional: Calculate and print summary statistics
for layer_idx, skipped_count in layer_skipped_patch_counts.items():
    total_skips = skipped_count.sum()
    avg_skips_per_patch = total_skips / num_patches
    max_skips = skipped_count.max()
    max_skip_patch = np.argmax(skipped_count)
    max_skip_patch_coords = (max_skip_patch % 14, max_skip_patch // 14)
    
    print(f"Layer {layer_idx} statistics:")
    print(f"  Total skipped patches: {total_skips}")
    print(f"  Average skips per patch: {avg_skips_per_patch:.2f}")
    print(f"  Most frequently skipped patch: {max_skip_patch} at position {max_skip_patch_coords} with {max_skips} skips")
    print()