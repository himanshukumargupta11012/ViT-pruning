...
class ModifiedViTLayer(ViTLayer):
    ...
    def forward(self, hidden_states: torch.Tensor,
        ...
        if not self.mlp_needed:
            if output_mask:
                self.boolean_mask = boolean_mask  # Store as an instance variable
        ...
        self.boolean_mask = boolean_mask  # Store boolean mask so it can be accessed outside
        ...
        if self.training or compute_cosine:
            ...
            self.true_labels = (similarity_val < self.sim_threshold).int().cpu().flatten()
            self.pred_labels = (mlp_output.squeeze(-1) > self.mlp_threshold).int().cpu().flatten()
            self.mlp_confusion_matrix = confusion_matrix(self.true_labels, self.pred_labels, labels=[0, 1])
