...
class ModifiedViTLayer(ViTLayer):
    def __init__(self, config, sim_threshold=0.9, mlp_threshold=0.5, mlp_needed=True):
        ...
        self.latent_size=16
        self.compress=CompressorEncoder(input_size=self.hidden_size, latent_size=self.latent_size)
        self.tokens = 196
        # self.mlp_layer=

        layer_sizes = [self.tokens * self.latent_size, 512, self.tokens]
    ...
    def forward(self, hidden_states: torch.Tensor,
        ...
        mlp_compressed = self.compress(mlp_input)
        mlp_compressed= mlp_compressed.view(mlp_compressed.size(0), -1)
        # print(mlp_compressed.shape)
        # print(self.hidden_size ,self.latent_size)
        mlp_output=self.mlp_layer(mlp_compressed)
...
class ModifiedViTModel(ViTModel):
    ...
    def vit_train(self):
        ...
        for layer in self.encoder.layer:
            ...
            if not hasattr(layer, 'compress'):
                continue
            
            for param in layer.mlp_layer.parameters():
                param.requires_grad = False
            
            for param in layer.compress.parameters():
                param.requires_grad = False
