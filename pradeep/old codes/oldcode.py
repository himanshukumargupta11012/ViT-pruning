...
def test(model, dataloader, full_testing=False, progress=True):
    ...
    with torch.no_grad():             # Disable gradient computation for efficiency
        ...
        for (inputs, labels) in dataloader:
            ...
            # Track the skip ratios from each layer
            for i, vit_layer in enumerate(model.encoder.layer):
                each_layer_skip[i] += vit_layer.skip_ratio.to("cpu")
...
pretrained_model_path = os.path.join('models', 'intermodel_himan.pth')
loss_type = 'cosine'
num_epochs = 5
lr = 1e-3
data_path = 'data'
...
torch.save(modified_vit_model.state_dict(), os.path.join('models', 'temp.pth'))
