...
class ModifiedViTLayer(ViTLayer):
    ...
    def forward(self, hidden_states: torch.Tensor,
        ...
        if self.training or compute_cosine:
            ...
            # self.loss = mlp_criterion(mlp_output.squeeze(-1), (similarity_val < self.threshold).float())
...
class CIFAR100Dataset(Dataset):
    def __init__(self, root, train=True, size=None, processor=None):
        self.dataset = torchvision.datasets.CIFAR100(root=root, train=train, download=False)           # Load CIFAR-100 dataset from torchvision
        if size is not None:
...
# Training function to train the model over multiple epochs
def trainNet(model, train_loader, test_loader, num_epochs=10, loss_type='both', lr=1e-4, progress=True):
    model.train()
    criterion = nn.CrossEntropyLoss()
    cosine_loss_ratio = 1.5
    ...
    for epoch in range(num_epochs):
            ...
            if(epoch%2==0):
                ...
                for i, vit_layer in enumerate(model.encoder.layer):
                    for param in vit_layer.mlp.parameters():
                        param.requires_grad = True

                    for param in vit_layer.cnn.parameters():
                        param.requires_grad = True

                    for param in vit_layer.update_net.parameters():
...
trainNet(modified_vit_model, train_loader, test_loader, num_epochs=num_epochs, loss_type=loss_type, lr=lr, progress=True)
