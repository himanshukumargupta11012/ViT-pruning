...
class ModifiedViTLayer(ViTLayer):
    def __init__(self, config, sim_threshold=0.9, mlp_threshold=0.5, mlp_needed=True):
        ...
        self.updateNet=UpdateNet()
        self.gamma=0.54
        self.lossloss=0
        self.updatenetloss=0
        self.mlploss=0
    ...
    def forward(self, hidden_states: torch.Tensor,
        ...
        hidden_states_without_cls = hidden_states[:, 1:]
        cls_token = hidden_states[:, 0:1]

        # print(cls_token.shape, hidden_states_without_cls.shape)
        # torch.Size([64, 1, 768]) torch.Size([64, 196, 768])
        # torch.Size([64, 196, 2*768])
        # # output_with_cls = torch.cat((hidden_states_without_cls, cls_token), dim=1)
        # output_with_cls = torch.cat((hidden_states_without_cls, cls_token), dim=-1)

        # cls_token_repeated = cls_token.repeat(1, hidden_states_without_cls.size(1), 1)
        # output_with_cls = torch.cat((hidden_states_without_cls, cls_token_repeated), dim=-1)



        # print(output_with_cls.shape)

        # mlp_output = self.mlp_layer(output_with_cls)
        mlp_output = self.mlp_layer(hidden_states[:,1:])
        ...
        # what if we add some error: 

        # type:1 get the error as follows: 
        # train a network, taking curr patch 768 and cls 768 as input, and outputing a new updated patch
        # type:2 directly add some averaged patch or so

        # TYPE 1
        # 
        # print('###########')
        # print(output.shape,boolean_mask.shape)
        # print('----')
        # self.updateNet(output,) 
        ...
        updated_patches = self.updateNet(input_to_update_net)
        ...
        if self.training or compute_cosine:
            ...
            # loss_from_error=real_output,output
            # print(real_output.shape,output.shape,boolean_mask[0][0])

            loss_fn = nn.MSELoss()

            # batch_size, num_tokens, embed_dim = 64, 197, 768
            # real_output = torch.randn(batch_size, num_tokens - 1, embed_dim, device='cuda')
            # output = torch.randn(batch_size, num_tokens, embed_dim, device='cuda')
            # boolean_mask = torch.randint(0, 2, (batch_size, num_tokens), dtype=torch.bool, device='cuda')
            ...
            # tensor(0.0027, device='cuda:2', grad_fn=<BinaryCrossEntropyBackward0>) tensor(1.5791e-05, device='cuda:2')
            self.loss = self.gamma * loss_mlp +1000*loss_updatenet*(1-self.gamma)
            # print(loss_updatenet)
            self.updatenetloss+=loss_updatenet
            self.mlploss+=loss_mlp
            self.lossloss+=self.loss
            ...
            # hidden_states[:,]
            # find places where model said 1, and difference b/w cls and that patch

            # cls_token = hidden_states[:, 0, :].unsqueeze(1)
            # avg_token = hidden_states[:, 1:, :].mean(dim=1).unsqueeze(1)

            # cls_token_norm = F.normalize(cls_token, p=2, dim=-1)
            # avg_token_norm = F.normalize(avg_token, p=2, dim=-1)

            # cosine_sim = torch.bmm(avg_token_norm, cls_token_norm.transpose(1, 2)).squeeze(2)
            # l2_dist = torch.norm(avg_token - cls_token, p=2, dim=-1)
            # l2_dist_squared = l2_dist ** 2
            ...
            # print("Avg Squared L2 Distance: ", avg_l2_dist_squared)
    ...
    def lossprint(self):
        # print(f"loss: {self.lossloss}")
        # print(f"updatenet loss: {self.updatenetloss}")
        # print(f"mlp loss: {self.mlploss}")
        # self.updatenetloss=0
        # self.mlploss=0
        # self.lossloss=0
        return self.lossloss,self.updatenetloss,self.mlploss
    def clearIt(self):
        self.lossloss = 0
        self.updatenetloss = 0
        self.mlploss = 0
    ...
    def vit_train(self):
        ...
        for layer in self.encoder.layer:
            if not hasattr(layer, 'mlp_layer'):
                continue
            if not hasattr(layer, 'updateNet'):
                continue
...
def train(model, train_loader, test_loader, save_path, num_epochs=10, loss_type='both', lr=1e-4):
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (inputs, labels) in enumerate(train_loader):
            ...
            if loss_type in ['cosine', 'both']:
                ...
                for i, vit_layer in enumerate(model.encoder.layer):
                    ...
                    loss11,loss21,loss31=vit_layer.lossprint()
                    loss1+=loss11
                    loss2+=loss21
                    loss3+=loss31
                    vit_layer.clearIt()
        ...
        # write_N_print(f"UpdateNet loss {epoch + 1} epochs: {LOSSUPDATENET}")
        # LOSSUPDATENET=0
...
if loss_type == "cosine":
    ...
    modified_vit_model.update_mlp_train()
