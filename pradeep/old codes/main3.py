...
class ModifiedViTLayer(ViTLayer):
    ...
    def forward(self, hidden_states: torch.Tensor,
        ...
        cls_token_repeated = cls_token.repeat(1, patch_tokens.size(1), 1)
        concatenated_tokens = torch.cat((cls_token_repeated, patch_tokens), dim=-1)
        # mlp_output = self.mlp_layer(hidden_states[:, 1:])
        mlp_output=self.mlp_layer(concatenated_tokens)
