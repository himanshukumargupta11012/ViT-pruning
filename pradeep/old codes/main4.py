# This code uses a single MLP for all the layers
# It uses the cosine similarity between the output of the layer and the output of the model to train the MLP
...
class SingleMLP(nn.Module):
    def __init__(self, input_size=768*2, hidden_size=512, output_size=1):
        super(SingleMLP, self).__init__()

        # Define the layers for MLP
    ...
    def forward(self, x):
        ...
        x = self.fc3(x)  # Output layer (no activation here, for regression)
        return x
...
class ModifiedViTLayer(ViTLayer):
    def __init__(self, config, sim_threshold=0.9, mlp_threshold=0.5,shared_mlp=None, mlp_needed=True):
        ...
        if not self.mlp_needed:
            return
        # self.mlp_layer=shared_ml
        if shared_mlp is None:
            self.mlp_layer = SingleMLP(input_size=config.hidden_size*2, hidden_size=64, output_size=1)
        else:
            self.mlp_layer = shared_mlp
...
shared_mlp = SingleMLP(input_size=768*2, hidden_size=64, output_size=1)
...
class ModifiedViTEncoder(ViTEncoder):
    def __init__(self, config: ViTConfig, sim_threshold=0.9, mlp_threshold=0.5):
        ...
        self.layer = nn.ModuleList([ModifiedViTLayer(config, sim_threshold, mlp_threshold,shared_mlp, mlp_needed_arr[_],) for _ in range(config.num_hidden_layers)])
        
        # for i, layer in enumerate(self.layer):
        #     print(f"Layer {i} - MLP memory address: {id(layer.mlp_layer)}")
...
class ModifiedViTModel(ViTModel):
    ...
    def vit_train(self):
        ...
        for layer in self.encoder.layer:
            ...
            for param in shared_mlp.parameters():
                param.requires_grad = False
...
if __name__ == '__main__':
    ...
    # print(shared_mlp.requires_grad)
    for param in shared_mlp.parameters():
