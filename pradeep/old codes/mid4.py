...
class ModifiedViTLayer(ViTLayer):
    def __init__(self, config, updatenetbackprop=False,sim_threshold=0.9, mlp_threshold=0.5, mlp_needed=True):
        super().__init__(config)
        # self.update_net = nn.Linear(config.hidden_size*2, config.hidden_size)
        ...
        self.updatenetbackprop=updatenetbackprop
        


    def forward(self, hidden_states: torch.Tensor,
        head_mask: Optional[torch.Tensor] = None,
        output_attentions: bool = False,updatenetbackprop=False, compute_cosine = False, output_mask=False):
        ...
        # output batch_size x num_patches x hidden_size
        # boolean_mask batch_size x num_patches
        # update the patch

        # when ever the boolean mask is false, calculate the average of the paches and update the patch
        # MHSA
        batch_avg = output.mean(dim=1, keepdim=True)  
        output[~boolean_mask] = batch_avg.expand(-1, output.shape[1], -1)[~boolean_mask] 
        

        # FFNN
        # output[~boolean_mask] = super().forward(hidden_states[~boolean_mask])[0]
        ...
        if self.training or compute_cosine:
            ...
            cls_tokens = output[:, 0]  # Shape: (batch_size, hidden_size)

        # Get indices where mask is False
            batch_indices, patch_indices = torch.where(~boolean_mask)
            # Create a copy of output to avoid inplace modification
            updated_output = output.clone()

            for b, p in zip(batch_indices, patch_indices):
                combined_input = torch.cat((cls_tokens[b], output[b, p]), dim=-1)  # Concatenation
                updated_output[b, p] = self.update_net(combined_input)  # Update using the model

            # Replace output with updated_output (avoid inplace modification)
            output = updated_output

            # for b, p in zip(batch_indices, patch_indices):
            #     combined_input = torch.cat((cls_tokens[b], output[b, p]), dim=-1)  # Shape: (2 * hidden_size)
            #     output[b, p] = self.update_net(combined_input)  # Pass concatenated tensor

                # output[b, p] = self.update_net(cls_tokens[b], output[b, p])  # Update patch
            ...
            # if(self.updatenetbackprop):
            loss_updatenet = loss_fn(F.normalize(output[:, 1:], p=2, dim=-1), F.normalize(real_output, p=2, dim=-1))
            updatenetloss=loss_updatenet
            ...
            self.loss=updatenetloss+mlploss
...
class ModifiedViTEncoder(ViTEncoder):
    def __init__(self, config: ViTConfig, updatenetbackprop,sim_threshold=0.9, mlp_threshold=0.5):
        super().__init__(config)
        self.updatenetbackprop=updatenetbackprop
        ...
        self.layer = nn.ModuleList([ModifiedViTLayer(config, updatenetbackprop,sim_threshold, mlp_threshold, mlp_needed_arr[_]) for _ in range(config.num_hidden_layers)])
    ...
    def forward(
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                layer_outputs = layer_module(hidden_states, layer_head_mask, output_attentions,updatenetbackprop=self.updatenetbackprop, compute_cosine=compute_cosine, output_mask=output_mask)
...
class ModifiedViTModel(ViTModel):
    def __init__(self, config: ViTConfig, updatenetbackprop,sim_threshold=0.9, mlp_threshold=0.5):
        super().__init__(config)
        self.updatenetbackprop=updatenetbackprop
        self.encoder = ModifiedViTEncoder(config,self.updatenetbackprop, sim_threshold, mlp_threshold)
        self.classifier = nn.Linear(config.hidden_size, config.num_labels)

    def updateVar(self,updatenetbackprop):
        self.updatenetbackprop=updatenetbackprop
        for layer in self.encoder.layer:
            layer.updatenetbackprop=updatenetbackprop
        for layer in self.encoder.layer:
            for param in layer.update_net.parameters():
                param.requires_grad = updatenetbackprop
...
def train(model, train_loader, test_loader, save_path, num_epochs=10, loss_type='both', lr=1e-4,updatenetbackprop=False):
    model.train()
    ...
    for epoch in range(num_epochs):
        ...
        model.updateVar(updatenetbackprop)
        updatenetbackprop=not updatenetbackprop
        model.vit_train()
...
updatenetbackprop=False
modified_vit_model = ModifiedViTModel(config,updatenetbackprop, sim_threshold, mlp_threshold)
...
if __name__ == '__main__':
    ...
    train(modified_vit_model, train_loader, test_loader, num_epochs=num_epochs, loss_type=loss_type, lr=lr, save_path=save_path,updatenetbackprop=updatenetbackprop)
    
    modified_vit_model.vit_train()

    train(modified_vit_model, train_loader, test_loader, num_epochs=5, loss_type="classification", lr=lr, save_path=save_path,updatenetbackprop=updatenetbackprop)
