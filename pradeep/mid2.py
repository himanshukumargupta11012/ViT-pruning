...
class ModifiedViTLayer(ViTLayer):
    def __init__(self, config,skip_patches=True, sim_threshold=0.9, mlp_threshold=0.5, mlp_needed=True):
    ...
    def forward(self, hidden_states: torch.Tensor,
        ...
        if self.skip_patches:

            self.mlp_accuracy_arr = torch.zeros(0)
            self.mlp_confusion_matrix = torch.ones(2, 2)
        ...
        if self.training or compute_cosine:
            ...
            if(self.skip_patches==False):
                
                # self.loss =torch.tensor(0.0,_requires_grad=True).to(device)

                self.mlp_accuracy_arr = torch.zeros(0)
                # self.mlp_confusion_matrix = torch.zeros(2, 2)
            ...
            self.loss = (focal_weight * nn.BCELoss(reduction='none')(pt, target)).mean()
...
class ModifiedViTEncoder(ViTEncoder):
    def __init__(self, config: ViTConfig, sim_threshold=0.9, mlp_threshold=0.5):
        ...
        for layer in range(config.num_hidden_layers):
            if(layer%2==0):
                # dont skip anyting
                self.layer.append(ModifiedViTLayer(config,False, sim_threshold, mlp_threshold, mlp_needed_arr[layer]))
            else:
                # skip if needed
                self.layer.append(ModifiedViTLayer(config,True, sim_threshold, mlp_threshold, mlp_needed_arr[layer]))    
...
def train(model, train_loader, test_loader, save_path, num_epochs=10, loss_type='both', lr=1e-4):
    ...
    for epoch in range(num_epochs):
        ...
        if(loss_type == 'cosine'):
            for params in vit_layer.parameters():
                params.requires_grad = True
                # for i, vit_layer in enumerate(model.encoder.layer):
                #     # if i%2==0:
                #         for params in vit_layer.parameters():
                #             params.requires_grad = False
                # for i, vit_layer in enumerate(model.e):
                    # if i%2==0:
                    #     for params in vit_layer.parameters():
...
if __name__ == '__main__':
    ...
    train(modified_vit_model, train_loader, test_loader, num_epochs=num_epochs, loss_type="classification", lr=lr, save_path=save_path)
