...
import os, sys
from transformers.models.vit.modeling_vit import ViTModel, ViTLayer, ViTEncoder, \
    ViTConfig, ViTAttention, prune_linear_layer, find_pruneable_heads_and_indices, \
...
class ModifiedViTAttention(ViTAttention):
    def __init__(self, config: ViTConfig) -> None:
    ...
    def forward(
        ...
        find_k_values: bool = False,
    ) -> Union[Tuple[torch.Tensor, torch.Tensor], Tuple[torch.Tensor]]:
        self_outputs = self.attention(hidden_states, head_mask, output_attentions, find_k_values)
...
class ViTSdpaSelfAttention(ViTSelfAttention):
    ...
    def forward(
        self, hidden_states, head_mask: Optional[torch.Tensor] = None, find_k_values=False ,output_attentions: bool = False
    ) -> Union[Tuple[torch.Tensor, torch.Tensor], Tuple[torch.Tensor]]:
        mixed_query_layer = self.query(hidden_states)
        ...
        # print('hello')
        # getting the next key values
        next_key_layer=key_layer.mean(dim=1)
        if(find_k_values):
            print(next_key_layer.shape)
            return (None, None, next_key_layer)
        ...
        return context_layer, None, next_key_layer



class ViTSdpaAttention(ModifiedViTAttention):
    def __init__(self, config: ViTConfig) -> None:
        super().__init__(config)
...
class ViTLayer(nn.Module):
    ...
    def forward(
        ...
        hidden_states: torch.Tensor,
        head_mask: Optional[torch.Tensor] = None,
        output_attentions: bool = False,find_k_values=False,
    ) -> Union[Tuple[torch.Tensor, torch.Tensor], Tuple[torch.Tensor]]:
        self_attention_outputs = self.attention(
            self.layernorm_before(hidden_states),  # in ViT, layernorm is applied before self-attention
            head_mask,
            output_attentions=output_attentions,
            find_k_values=find_k_values
        )
...
def MaskIt(tensor1, tensor2, threshold=0.95):

    # Normalize both tensors along the last dimension
    tensor1_norm = F.normalize(tensor1, p=2, dim=-1)
    tensor2_norm = F.normalize(tensor2, p=2, dim=-1)

    # Compute cosine similarity
    similarity = (tensor1_norm * tensor2_norm).sum(dim=-1)  # shape: (batch_size, num_patches)

    # Generate mask
    mask = similarity > threshold  # shape: (batch_size, num_patches)
...
class ModifiedViTLayer(ViTLayer):
    ...
    def forward(self, hidden_states: torch.Tensor,
        ...
        # finding K_x^: 
        output_layer = super().forward(hidden_states[:, 0:])[0]
        key_next_layer = super().forward(output_layer[:, 0:], find_k_values=True)[2]
        
        # finding current layer 
        key_layer = super().forward(hidden_states[:, 0:], find_k_values=True)[2]
         
        # print('a')
        # print(key_next_layer.shape)
        # print(key_layer.shape)
        # print('b')
        
        # boolean_mask = (mlp_output >= self.mlp_threshold).squeeze(-1)
        boolean_mask = MaskIt(key_next_layer, key_layer, threshold=self.sim_threshold)
        # print(real_output)
        # print(real_output[0].shape, real_output[1],real_output[2].shape)

        # mask=torch.ones((hidden_states.shape[0], hidden_states.shape[1]), dtype=torch.bool).to(device)
        ...
        self.skiped = torch.sum(boolean_mask).item()
