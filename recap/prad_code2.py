...
class ViTSelfAttention(nn.Module):
    ...
    def forward(
        ...
        return outputs
import numpy as np
result_map = {i: [] for i in range(197)}  # {1: [[],[]], 2: [], ...}
def fillmap(array1, array2):
    # Iterate over the array2
    
    for idx, is_true in enumerate(array2):
        if is_true:
        #     print(array1.shape)
            for i in range(array1.size(1)):  # Iterate over each key in result_map
                
                # Create a list with the current value
                
                curr_att = np.array(array1[0, i, idx].item())  # A list containing the single value
                
                result_map[i].append([curr_att])  # Append this list to the key `i`
...
class ModifiedViTSelfAttention(nn.Module):
    ...
    def forward(
        ...
        num_false = torch.sum(~patch_indices_to_keep).item()
        # if
        # print(num_false)
        fillmap(attention_map, patch_indices_to_keep)
        # print(len(result_map[3]))        
        return outputs
...
# class DHSLayer(ModifiedViTLayer):
#     def __init__(self, config, threshold=0.03):
#         super().__init__(config)
#         self.hidden_size = config.hidden_size
#         layer_sizes = [self.hidden_size, 256, 64, 1]
...
#     def forward(self, hidden_states: torch.Tensor,
#         head_mask: Optional[torch.Tensor] = None,
#         output_attentions: bool = False, compute_cosine = False):
...
# class ModifiedViTEncoder(ViTEncoder):
#     def __init__(self, config: ViTConfig, threshold=0.03):
#         super().__init__(config)
#         self.layer = nn.ModuleList([DHSLayer(config, threshold) for _ in range(config.num_hidden_layers)])
...
# print(f'Test accuracy at starting: {test(modified_vit_model, test_loader, full_testing=False, progress=True)}')
