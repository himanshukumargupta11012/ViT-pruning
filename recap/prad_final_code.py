...
#     def __init__(self, config, threshold=0.05, beta=0.5, similarity_threshold=0.5, mlp_threshold=0.5):
#         super().__init__(config)
#         self.hidden_size = config.hidden_size
        
#         # Define the MLP layers with a 768 intermediate dimension
#         layer_sizes = [self.hidden_size, 128, 768, 64, 1]
...
#         layer3_output = self.mlp_layer[5](self.mlp_layer[4](middle_layer_output))
#         mlp_output = self.mlp_layer[7](self.mlp_layer[6](layer3_output))
        
#         # Binary mask creation based on the MLP output
#         # mlp_output = self.mlp_layer[2](layer2_output)  # After 768 layer, before 64 layer
...
#         # print(boolean_mask.shape,cls_col.shape)
#         boolean_mask = torch.cat((cls_col, boolean_mask), dim=1)

#         # Clone the hidden states to 'output' for modification based on the boolean mask
#         output = hidden_states.clone()
#         batch_size = hidden_states.shape[0]

#         # Update the output based on the mask
#         # for i in range(batch_size):
#         #     output[i][boolean_mask[i]] = super().forward(hidden_states[i][boolean_mask[i]].unsqueeze(0))[0].squeeze(0)
        
        
#         # temp = torch.ones((hidden_states.shape[0], 1), dtype=torch.bool).to(boolean_mask.device)
#         # middle_layer_output = torch.cat((temp, middle_layer_output), dim=1)
#         ones = torch.ones((middle_layer_output.size(0), 1, middle_layer_output.size(2)), device=middle_layer_output.device)

#         # Concatenate the ones tensor at the beginning (along the sequence dimension)
#         middle_layer_output_new = torch.cat((ones, middle_layer_output), dim=1)
# # torch.Size([196, 768])
#         for i in range(batch_size):
#             output_parent=super().forward(hidden_states[i][boolean_mask[i]].unsqueeze(0))[0].squeeze(0)
#             output[i][boolean_mask[i]] = output_parent
...
#                 cos_similarity = (F.cosine_similarity(real_output, middle_layer_output, dim=-1) + 1) / 2
...
#                 mse_loss = F.mse_loss(middle_layer_output, hidden_states[:, 1:])  # MSE loss between input and middle layer output

#                 # Final loss as a weighted sum of BCE and MSE losses
#                 self.loss = self.beta * bce_loss + (1 - self.beta) * mse_loss
...
for layer_idx, layer in enumerate(modified_vit_model.encoder.layer):
    print(f"\nChecking layer {layer_idx}:")
    
    # Print all attributes and parameters inside the layer
    for name, module in layer.named_children():
        ...
        for param_idx, param in enumerate(module.parameters()):
            print(f"    Parameter {param_idx}: requires_grad = {param.requires_grad}")
            print(f"    Shape: {param.shape}") # or param.detach().cpu().numpy() to print it as a NumPy array if desired

    # # Specifically checking for the sub-modules (ScoreNetMLP, ScoreNetconv1, ScoreNetconv2, ScoreNetMLP2)
    # if hasattr(layer, 'ScoreNetMLP'):
    #     print("\n  ScoreNetMLP Parameters:")
    #     for param_idx, param in enumerate(layer.ScoreNetMLP.parameters()):
    #         print(f"    Parameter {param_idx}: requires_grad = {param.requires_grad}")
    #         print(f"    Shape: {param.shape}")
    
    # if hasattr(layer, 'ScoreNetconv1'):
    #     print("\n  ScoreNetconv1 Parameters:")
    #     for param_idx, param in enumerate(layer.ScoreNetconv1.parameters()):
    #         print(f"    Parameter {param_idx}: requires_grad = {param.requires_grad}")
    #         print(f"    Shape: {param.shape}")
    
    # if hasattr(layer, 'ScoreNetconv2'):
    #     print("\n  ScoreNetconv2 Parameters:")
    #     for param_idx, param in enumerate(layer.ScoreNetconv2.parameters()):
    #         print(f"    Parameter {param_idx}: requires_grad = {param.requires_grad}")
    #         print(f"    Shape: {param.shape}")
    
    # if hasattr(layer, 'ScoreNetMLP2'):
    #     print("\n  ScoreNetMLP2 Parameters:")
    #     for param_idx, param in enumerate(layer.ScoreNetMLP2.parameters()):
    #         print(f"    Parameter {param_idx}: requires_grad = {param.requires_grad}")
    #         print(f"    Shape: {param.shape}")
