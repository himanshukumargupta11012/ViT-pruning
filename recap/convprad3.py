...
class SimpleCNN(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(1, 8, kernel_size=4)  # Input channels = 1
        self.conv2 = nn.Conv2d(8, 16, kernel_size=4)
        self.conv3 = nn.Conv2d(16, 1, kernel_size=4)  # Output channels = 1
    ...
    def forward(self, x):
        ...
        print(x.shape)
        x = self.conv3(x)  # Output shape: (batch_size, 1, 14, 14)
        print(x.shape)
...
class PatchScoringModel(nn.Module):
    ...
    def forward(self, x):
        x = self.mlp(x)  
        batch_size = x.size(0)
        x = x.view(batch_size, 1, 14, 14)  # Reshape to (batch_size, 1, 14, 14)
...
class DHSLayer(ModifiedViTLayer):
    ...
    def forward(self, hidden_states: torch.Tensor,
        ...
        batch_size = hidden_states.shape[0]
        for i in range(batch_size):
            # locations where changes are to be made
            output[i][boolean_mask[i]] = super().forward(hidden_states[i].unsqueeze(0),patch_indices_to_keep=boolean_mask[i])[0].squeeze(0)
            # error_term_i =hidden_states[i][0] / torch.norm(hidden_states[i][0], p=2)
            
            # output[i][~boolean_mask[i]] = hidden_states[i][~boolean_mask[i]]+error_term_i
        ...
        if self.training or compute_cosine:
            ...
            alpha = 0.5
            similarity_val = alpha * cos_similarity + (1 - alpha) * dist_similarity   # mean_real_output = real_output.mean(dim=0)
...
#             # locations where changes are not made
#             # try making change
#             # error_term_i = hidden_states[i][0].unsqueeze(0) - hidden_states[i][1:].mean(dim=1).unsqueeze(0)
#             error_term_i =hidden_states[i][0] / torch.norm(hidden_states[i][0], p=2)
            
#             # print(hidden_states[i][0].unsqueeze(0).shape)
#             # print(hidden_states[i][0].unsqueeze(0).shape)
#             output[i][~boolean_mask[i]] = hidden_states[i][~boolean_mask[i]]+error_term_i
